...
async function main() {
  ...
  for (const c of demoCandidats) {
    await prisma.candidat.create({
      data: {
        ...
        telephone: "06 12 34 56 78", ville: c.ville, persona: c.persona,
        etapePipeline: c.etape, statut: c.etape >= 13 ? "place" : "en_cours",
        sourceEntree: "Formulaire site web", entiteId: entites[c.entite],
  ...
  for (const e of demoEntreprises) {
    const ent = await prisma.entreprise.create({
      data: {
        raisonSociale: e.raison, siret: e.siret, taille: e.taille, persona: e.persona,
        secteur: e.secteur, ville: e.ville, etapePipeline: e.etape,
  ...
  for (const c of candidats.slice(0, 4)) {
    await prisma.tache.create({
      data: { titre: `Appel de qualification — ${c.prenom} ${c.nom}`, type: "appel", priorite: "haute", candidatId: c.id, echeance: new Date(Date.now() + 24 * 3600 * 1000) },
    });
  }
  for (const e of entreprises.slice(0, 4)) {
    await prisma.tache.create({
      data: { titre: `Suivi commercial — ${e.raisonSociale}`, type: "email", priorite: "normale", entrepriseId: e.id, echeance: new Date(Date.now() + 48 * 3600 * 1000) },
    });
  }

  await prisma.evenement.create({
    data: { titre: "Job Dating Commerce — Paris", type: "job_dating", date: new Date(Date.now() + 14 * 24 * 3600 * 1000), lieu: "DBS Paris — 75009", entiteId: entites["DBS"] },
  });
  await prisma.evenement.create({
    data: { titre: "Journée d'information collective — Petite Enfance", type: "jic", date: new Date(Date.now() + 7 * 24 * 3600 * 1000), lieu: "PBA Paris", entiteId: entites["PBA"] },
  });
  ...
  for (const c of allCandidats.slice(0, 4)) {
    await prisma.note.create({
      data: { contenu: `Premier contact téléphonique avec ${c.prenom}. Très motivé(e), à recontacter sous 48h.`, auteur: "Sophie Martin", candidatId: c.id },
    });
    await prisma.activite.create({
      data: { type: "note", titre: "Note ajoutée", contenu: `Premier contact téléphonique avec ${c.prenom}.`, auteur: "Sophie Martin", candidatId: c.id },
    });
    await prisma.activite.create({
      data: { type: "email_envoye", titre: "Email de bienvenue envoyé", contenu: "Template : Bienvenue + fiche besoin", auteur: "Système", candidatId: c.id },
    });
  }
  ...
  for (const c of allCandidats.slice(0, 3)) {
    await prisma.meeting.create({
      data: {
        ...
        lieu: "Visio Google Meet", ownerName: "Sophie Martin", candidatId: c.id,
      },
    });
  }

  await prisma.ticket.create({
    data: { sujet: "Problème d'accès à la plateforme pédagogique", description: "L'apprenant ne reçoit pas l'email de connexion à l'espace LMS.", categorie: "technique", priorite: "haute", statut: "en_cours", source: "email", candidatId: allCandidats[0]?.id },
  });
  await prisma.ticket.create({
    data: { sujet: "Demande d'attestation de scolarité", description: "L'entreprise demande une attestation pour le dossier OPCO.", categorie: "administratif", priorite: "normale", statut: "nouveau", source: "telephone", entrepriseId: allEntreprises[0]?.id },
  });
  ...
  await prisma.liste.createMany({
    data: [
      { nom: "Candidats Bac+5 sans entreprise", objet: "candidat", filtres: JSON.stringify([{ champ: "etapePipeline", operateur: "lt", valeur: 13 }]) },
      { nom: "Entreprises partenaires actives PNFB", objet: "entreprise", filtres: JSON.stringify([{ champ: "statut", operateur: "eq", valeur: "partenaire_actif" }]) },
  ...
  await prisma.emailTemplate.createMany({
    data: [
      { nom: "Bienvenue candidat", sujet: "Bienvenue chez {{entite}}", contenu: "<p>Bonjour {{prenom}},</p><p>Merci pour votre candidature.</p>", categorie: "bienvenue", variables: "prenom,entite,formation" },
      { nom: "Prospection entreprise", sujet: "Recrutez en alternance", contenu: "<p>Bonjour,</p><p>Profils dispo {{secteur}}.</p>", categorie: "prospection", variables: "secteur,raisonSociale" },
      { nom: "Relance fiche besoin", sujet: "Avez-vous reçu notre fiche besoin ?", contenu: "<p>Bonjour {{contact}},</p><p>Petit rappel.</p>", categorie: "relance", variables: "contact,raisonSociale" },
    ],
  });
  ...
  await prisma.workflow.createMany({
    data: [
      { nom: "Bienvenue automatique candidat", declencheur: "candidat.cree", objet: "candidat", actions: JSON.stringify([{ type: "envoyer_email", params: { templateNom: "Bienvenue candidat" } }, { type: "creer_tache", params: { titre: "Appel de qualification", priorite: "haute" } }]), nbExecutions: 12 },
      { nom: "Notif placement candidat", declencheur: "candidat.etape_changee", objet: "candidat", conditions: JSON.stringify([{ champ: "etape", operateur: "eq", valeur: 13 }]), actions: JSON.stringify([{ type: "envoyer_email", params: { template: "Félicitations" } }]), nbExecutions: 4 },
      { nom: "Relance entreprise inactive", declencheur: "entreprise.inactive_30j", objet: "entreprise", actions: JSON.stringify([{ type: "envoyer_email", params: { templateNom: "Prospection entreprise" } }]), nbExecutions: 7 },
  ...
  await prisma.sequence.create({
    data: {
      ...
      description: "Séquence automatique de bienvenue sur 14 jours",
      cibleObjet: "candidat",
      etapes: JSON.stringify([
        ...
        { ordre: 5, jourOffset: 14, sujet: "Réservez votre entretien", contenu: "CTA entretien" },
      ]),
    },
  });
  await prisma.sequence.create({
    data: {
      ...
      description: "Réactiver les entreprises inactives depuis 6 mois",
      cibleObjet: "entreprise",
      etapes: JSON.stringify([
        { ordre: 1, jourOffset: 0, sujet: "Nouveaux profils disponibles", contenu: "Pitch" },
        { ordre: 2, jourOffset: 4, sujet: "5 profils Bac+2 commerce dispo", contenu: "Liste profils" },
        { ordre: 3, jourOffset: 10, sujet: "On vous appelle ?", contenu: "CTA RDV" },
      ]),
    },
  });
  ...
  await prisma.campaign.createMany({
    data: [
      { nom: "Newsletter mensuelle candidats - Mai", sujet: "Le mag du mois", contenu: "<h1>Newsletter Mai</h1>", statut: "envoyee", dateEnvoi: new Date(Date.now() - 5 * 24 * 3600 * 1000), nbDestinataires: 1247, nbOuvertures: 423, nbClics: 89 },
      { nom: "Job Dating Commerce - Invitation entreprises", sujet: "Job Dating Paris — 30 candidats commerce dispo", contenu: "<h1>Invitation Job Dating</h1>", statut: "planifiee", dateEnvoi: new Date(Date.now() + 2 * 24 * 3600 * 1000), nbDestinataires: 156 },
      { nom: "Veille - Nouvelles aides 2026", sujet: "Réforme : aides employeur revalorisées", contenu: "<h1>Veille</h1>", statut: "brouillon" },
    ],
  });
  ...
  await prisma.quote.create({
    data: {
      ...
      totalHT: 8550, tva: 0, totalTTC: 8550, statut: "envoye",
  ...
  await prisma.quote.create({
    data: {
      ...
      totalHT: 30000, tva: 0, totalTTC: 30000, statut: "accepte",
